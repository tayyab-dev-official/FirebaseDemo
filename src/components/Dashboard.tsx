...
export default function Dashboard() {
    ...
    const { 
        ...
        setUser, 
        updateProfile, 
        setUpdateProfile } = useAuth()

    const photoURL = user?.photoURL || "" 

    const moodEls = moods.map(
      (mood) => {
        ...
        return (
          <button
            ...
            className={`flex flex-col justify-center gap-4 p-2 transition-all duration-1000 ease-in-out ${isActive ? "scale-125 bg-blue-300 opacity-100 rounded-md": "hover:scale-125  hover:opacity-100 opacity-70"}`}
          >
    ...
    const postsEl = posts.map((post) => {
      ...
      const postDate = !post.createdAt ? 'Getting Timestamp...'
        : post.createdAt
        .toDate()
    ...
    function getPost() {
      const postEl: HTMLInputElement | null = document.querySelector('#post-area')
      if (!postEl) {
        return ''
      }
